	...
	if (!atomic_read(&data_ready)) {
		...
		if (!atomic_read(&suspend_flag)) {
			...
			if (failure_count >= MAX_FAILURE_COUNT) {
				printk(KERN_ERR "AKM8975 AKECS_GetData: successive %d failure.\n", failure_count);
...
static void AKECS_SetYPR(short *rbuf)
{
	...
	if (atomic_read(&a_flag)) {
		...
		msm_sensors_input_report_value(EV_ABS, ABS_X, rbuf[6]);
		msm_sensors_input_report_value(EV_ABS, ABS_Y, rbuf[7]);
		msm_sensors_input_report_value(EV_ABS, ABS_Z, rbuf[8]);
		msm_sensors_input_report_value(EV_ABS, ABS_WHEEL, rbuf[5]);
	}
...
static int akm_self_test(void)
{
	...
	while (j--) {
		...
		if (50 == i) {
			printk("XXXXXXXXXXXXXXX  akm_self_test error XXXXXXXXXXXXXx");
			return -1;
		}
		...
		ret = AKI2C_RxData(buffer, SENSOR_DATA_SIZE);
		if (ret < 0) {
			printk(KERN_ERR "Self_test: I2C failed\n");
			return -1;
		}

		printk("SELF_TEST DATA:");
		for (i = 1; i < 7; i += 2) {
			printk(" %d,   ", *(short *)(buffer + i));
		}
		printk("\n");
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("XXXXXXXXXXXXXXXXakm8975_probe\n");
...
#ifdef SELF_TEST
	/*if into self_test mode, delay > 5000ms */
	err = akm_self_test();
	if (err < 0) {
		printk(KERN_ERR "AKM8975 akm8975_probe: self_test failed\n");
		goto exit3;
	}
#endif
...
#ifdef AKM8975C_INTERRUPT
	/* IRQ */
	err = request_irq(client->irq, akm8975_interrupt, IRQ_TYPE_EDGE_RISING, "akm8975_DRDY", akm);
	if (err < 0) {
	...
	msm_sensors_input_set_params(EV_ABS, ABS_RX, 0, 23040, 0, 0);
	/* pitch (-180, 180) */
	//input_set_abs_params(akm->input_dev, ABS_RY, -11520, 11520, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_RY, -11520, 11520, 0, 0);
	/* roll (-90, 90) */
	//input_set_abs_params(akm->input_dev, ABS_RZ, -5760, 5760, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_RZ, -5760, 5760, 0, 0);
	/* x-axis acceleration (720 x 8G) */
	//input_set_abs_params(akm->input_dev, ABS_X, -5760, 5760, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_X, -5760, 5760, 0, 0);
	/* y-axis acceleration (720 x 8G) */
	//input_set_abs_params(akm->input_dev, ABS_Y, -5760, 5760, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_Y, -5760, 5760, 0, 0);
	/* z-axis acceleration (720 x 8G) */
	//input_set_abs_params(akm->input_dev, ABS_Z, -5760, 5760, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_Z, -5760, 5760, 0, 0);
	/* temparature */
	/*
	...
	msm_sensors_input_set_params(EV_ABS, ABS_RUDDER, -32768, 3, 0, 0);
	/* status of acceleration sensor */
	//input_set_abs_params(akm->input_dev, ABS_WHEEL, -32768, 3, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_WHEEL, -32768, 3, 0, 0);
	/* x-axis of raw magnetic vector (-4096, 4095) */
	//input_set_abs_params(akm->input_dev, ABS_HAT0X, -20480, 20479, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_HAT0X, -20480, 20479, 0, 0);
	/* y-axis of raw magnetic vector (-4096, 4095) */
	//input_set_abs_params(akm->input_dev, ABS_HAT0Y, -20480, 20479, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_HAT0Y, -20480, 20479, 0, 0);
	/* z-axis of raw magnetic vector (-4096, 4095) */
	//input_set_abs_params(akm->input_dev, ABS_BRAKE, -20480, 20479, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_BRAKE, -20480, 20479, 0, 0);
	/* Set name */
	...
	if (err) {
		printk(KERN_ERR "AKM8975 akm8975_probe: akmd_device register failed\n");
		goto exit7;
	}

	err = misc_register(&akm_aot_device);
	if (err) {
		printk(KERN_ERR "AKM8975 akm8975_probe: akm_aot_device register failed\n");
		goto exit8;
	}
	...
	printk("XXXXXXXXXXXXXXXXsuccessfully probed.\n");
	return 0;
