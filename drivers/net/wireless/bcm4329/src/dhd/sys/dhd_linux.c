...
static dhd_pub_t * dhdp_test;
static int wake_lock_exist = 0;
static int wifi_resume(struct platform_device *pdev)
{
	if (wake_lock_exist)
	WAKE_LOCK_TIMEOUT(dhdp_test,WAKE_LOCK_TEST, HZ);
	DHD_TRACE(("##> %s\n", __FUNCTION__));
	return 0;
}
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	if (dhd && dhd->up) {
		if (value && dhd->in_suspend) {
				...
				rc = dhdcdc_set_ioctl(dhd, 0, WLC_SET_PM,
					(char *)&power_mode, sizeof(power_mode));
				if (rc < 0) {
					DHD_ERROR(("%s: power mode set failed",__FUNCTION__));
					return rc;
				}
				...
				*/
				...
				rc = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
				if (rc < 0) {
					DHD_ERROR(("%s: set bcn_li_dtim failed\n", __FUNCTION__));
					return rc;
				}
				...
				rc = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
				if (rc < 0) {
					DHD_ERROR(("%s: disable roam_off failed\n", __FUNCTION__));
					return rc;
				}
#endif /* CUSTOMER_HW2 */
				...
				power_mode = PM_FAST;
				rc = dhdcdc_set_ioctl(dhd, 0, WLC_SET_PM, (char *)&power_mode,
					sizeof(power_mode));
				if (rc < 0) {
					DHD_ERROR(("%s: set power mode pm_fast failed\n", __FUNCTION__ ));
					return rc;
				}
				...
				rc = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
				if (rc < 0) {
					DHD_ERROR(("%s: restore bcn_li_dtim failed\n", __FUNCTION__));
					return rc;
				}
#ifdef CUSTOMER_HW2
				...
				rc = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
				if (rc < 0) {
					DHD_ERROR(("%s: enable roam_off failed\n", __FUNCTION__));
					return rc;
				}
#endif /* CUSTOMER_HW2 */
			}
...
#if defined(JRD)
		WAKE_LOCK_TIMEOUT(&dhd->pub, WAKE_LOCK_JRD_TMOUT, (5*HZ));
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25)) && 1
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (jrd_wifi_hang) {
		printf("%s: ______wifi hang %d\n", __FUNCTION__, jrd_wifi_hang);
		bcmerror = -ETIMEDOUT;
		goto done;
	}
	if (ifidx == DHD_BAD_IF)
...
#endif
	if (g_onoff == 1) {
		printf("## wifi power off, not work now ##\n");
		return -EBUSY;
	}
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	WAKE_LOCK_INIT(&dhd->pub, WAKE_LOCK_TEST, "jrd-test");
#if defined(JRD)
	WAKE_LOCK_INIT(&dhd->pub, WAKE_LOCK_JRD_TMOUT, "wlan_jrd_timeout");
	WAKE_LOCK_INIT(&dhd->pub, WAKE_LOCK_JRD, "wlan_jrd");
#endif
	wake_lock_exist = 1;
#ifdef CONFIG_HAS_EARLYSUSPEND
	dhd->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
	dhd->early_suspend.suspend = dhd_early_suspend;
	dhd->early_suspend.resume = dhd_late_resume;
	register_early_suspend(&dhd->early_suspend);
#endif
    dhdp_test = &dhd->pub;//for test
	return &dhd->pub;
...
dhd_detach(dhd_pub_t *dhdp)
{
	...
	/* && defined(DHD_GPL) */
		...
		WAKE_LOCK_DESTROY(dhdp, WAKE_LOCK_TEST);
#if defined(JRD)
		WAKE_LOCK_DESTROY(dhdp, WAKE_LOCK_JRD);
		WAKE_LOCK_DESTROY(dhdp, WAKE_LOCK_JRD_TMOUT);
#endif
		free_netdev(ifp->net);
		MFREE(dhd->pub.osh, ifp, sizeof(*ifp));
...
#endif /* DHD_DEBUG */
#if defined(JRD)
int jrd_wake_lock(dhd_pub_t *pub)
{
    dhd_info_t *dhd = (dhd_info_t *)(pub->info);
//    unsigned long flags;
  //  int ret = 0;
...
int jrd_wake_unlock(dhd_pub_t *pub)
{
    dhd_info_t *dhd = (dhd_info_t *)(pub->info);
//  unsigned long flags;
    //int ret = 0;
...
//    dhd_os_wake_lock_timeout(pub);
	WAKE_LOCK_TIMEOUT(pub, WAKE_LOCK_JRD_TMOUT, 5*HZ);
    if (dhd) {
//        spin_lock_irqsave(&dhd->wl_lock, flags);
    //    if (dhd->wl_count) {
    //        dhd->wl_count--;
